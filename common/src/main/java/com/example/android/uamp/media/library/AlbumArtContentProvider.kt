...
internal class AlbumArtContentProvider : ContentProvider() {
    ...
    override fun openFile(uri: Uri, mode: String): ParcelFileDescriptor? {
        ...
        var file = File(context.cacheDir, uri.path?.removePrefix("/"))
        ...
        if (!file.exists()) {
            try {
                ...
                if (!cacheFile.renameTo(file)) {
                    Log.e("AlbumArtContentProvider", "Failed to rename file ${cacheFile.absolutePath} to ${file.absolutePath}")
                    file = cacheFile
                }
            } catch (e: Exception) {
                Log.e("AlbumArtContentProvider", "Failed to download album art", e)
                throw FileNotFoundException("Unable to download file for URI: $remoteUri")
            }
        }
